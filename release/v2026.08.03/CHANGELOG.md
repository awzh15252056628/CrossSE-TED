# CrossSE-TED publication-associated release v2026.08.03
...
- Gene-level differential-expression collection: **24 adjacent-stage contrasts from six taxa**.
- Five manuscript figures in SVG, PDF, 600-dpi PNG, and 600-dpi TIFF formats.
- Supplementary Tables S1-S7, including the 239-column coverage table, the 30-matrix checksum manifest, and the catalog audit.
...
- Excluded the *Picea abies* exon-feature DEG payload and its feature-level counts from the publication-associated gene-level DEG scope.
- Unified the DEG rule as `padj < 0.05` and `|log2FoldChange| >= 1` in the manuscript, supplementary files, and website.
- Removed the extended local-only scope from public-facing descriptions so that all release surfaces use the publication-associated query scope.
- Replaced the original image-based Figure 1 with a reproducible schematic limited to the ten manuscript-associated query taxa and the implemented data-to-service architecture.
- Removed the unsupported core-TF-composition panel from Figure 2; the revised figure reports only expression-specificity classes supported by available stage-level matrices.
...
- Website release date: **2026-08-03**.
- Manuscript access date: **2026-08-03**.
- Supplementary-material date: **2026-08-03**.
- Supplementary Table S4 stage mappings are documented without unverified independent-review fields.
