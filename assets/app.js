...
DB.figures.push(
  {group:"Manuscript core results",title:"Expression Specificity and Core-TF Composition",desc:"Four τ-based expression classes across species and core transcription-factor family composition.",size:"269 KB",link:"assets/Figure_2_expression_specificity_and_core_TF.png"},
  {group:"Manuscript core results",title:"Liriodendron hybrid Coexpression Network",desc:"Published L. hybrid network query result with SE-related regulators and functionally annotated candidate genes.",size:"855 KB",link:"assets/Figure_3_Liriodendron_coexpression_network.png"},
  {group:"Application case",title:"PsbO/OEE1 Microsynteny and Ka/Ks",desc:"Cross-species microsynteny and evolutionary-constraint evidence for the PsbO/OEE1 candidate family.",size:"493 KB",link:"assets/Figure_6_PsbO_OEE1_microsynteny_KaKs.png"}
);
...
function renderFigures() {
  ...
  $("figureGrid").innerHTML = rows.map((x,i) => `<article class="figure-card" data-i="${DB.figures.indexOf(x)}">
    <img src="${x.link}" alt="${x.title}" loading="lazy"><div class="fig-body"><h3>${x.title}</h3><p>${x.desc}</p><div class="tags" style="margin-top:8px"><span class="tag">${x.group}</span><span class="tag">${x.size}</span></div></div>
  </article>`).join("");
  document.querySelectorAll(".figure-card").forEach(card => card.addEventListener("click", () => openFigure(Number(card.dataset.i))));
}
...
const MODULE_META = [
  ...
  {id:"figures", title:"Figure Gallery", desc:"30 integrated result figures, including expression-specificity, coexpression and Ka/Ks results.", accent:"#d0416b", grad:"linear-gradient(135deg,#d0416b,#ef7396)", icon:"image"},
...
(function(){
  ...
  async function ensureSraData(){
    ...
    try{
      ...
      setSraStatus("Curated catalog not loaded (showing built-in data). Open via a local server or GitHub Pages to load datasets_data/sra_datasets.json.gz. "+err, "warn");
    }finally{
      SRA.loading = false;
    }
  }
